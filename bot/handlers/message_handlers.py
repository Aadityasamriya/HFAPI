    ...
    async def _handle_text_generation(update, context: ContextTypes.DEFAULT_TYPE, prompt: str, api_key: str, chat_history: list, routing_info: dict) -> None:
        ...
        try:
            ...
            if success and response:
                ...
                # Enhanced response formatting with model information
                model_used = routing_info.get('recommended_model', 'Unknown').split('/')[-1]  # Get model name only
    ...
    async def _handle_code_generation(update, context: ContextTypes.DEFAULT_TYPE, prompt: str, api_key: str, routing_info: dict) -> None:
        ...
        try:
            ...
            if success and code_response:
                model_used = routing_info.get('recommended_model', 'Unknown').split('/')[-1]
                response_text = f"💻 **Code Generated** ({language})\n\n{code_response}\n\n🎯 *Generated by {model_used} - Latest 2024-2025 coding AI*"
...
**Primary Emotion:** {emotion_name.title()} 
**Confidence:** {confidence}%

**All Detected Emotions:**
{chr(10).join([f"• {e.get('label', 'Unknown').title()}: {round(e.get('score', 0)*100, 1)}%" for e in emotions[:5]])}
