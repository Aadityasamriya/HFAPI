...
class PostgreSQLProvider(StorageProvider):
    ...
    async def connect(self) -> None:
        ...
        try:
            # Get database URL from environment (automatically set by Replit)
    ...
    async def _initialize_encryption(self, conn: Connection) -> None:
        ...
        try:
            # Try to get existing encryption seed
            result = await conn.fetchrow(
                "SELECT encrypted_seed FROM encryption_keys WHERE key_name = 'global_seed'"
            )
            
            if result:
                # Use existing seed
                self._global_seed = result['encrypted_seed']
                logger.info("🔐 Using existing encryption seed from database")
            else:
                # Generate new seed and store it
                self._global_seed = secrets.token_bytes(32)
                await conn.execute(
                    "INSERT INTO encryption_keys (key_name, encrypted_seed) VALUES ($1, $2)",
