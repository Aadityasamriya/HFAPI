...
class SupabaseUserProvider(StorageProvider):
    ...
    def __init__(self):
        ...
        if not SUPABASE_AVAILABLE:
            raise ImportError("Supabase dependencies not available. Install with: pip install asyncpg sqlalchemy[asyncio] psycopg2-binary")
    ...
    async def connect(self) -> None:
        """Establish connection to Supabase management database"""
        try:
            # Get Supabase URLs from configuration
            self.mgmt_database_url = os.getenv('SUPABASE_MGMT_URL')
            self.base_user_db_url = os.getenv('SUPABASE_USER_BASE_URL')
            
            if not self.mgmt_database_url:
                raise ValueError("SUPABASE_MGMT_URL is not configured")
            
            if not self.base_user_db_url:
            ...
            # Create management database connection
    ...
    async def _get_user_engine(self, user_id: int) -> Any:
        ...
        user_db_url = f"{self.base_user_db_url}?options=-csearch_path%3D{schema_name}"
    ...
    def _secure_delete(self, sensitive_data: Any) -> None:
        ...
        try:
            ...
            elif isinstance(sensitive_data, bytes):
                # For bytes, we can at least try to clear
